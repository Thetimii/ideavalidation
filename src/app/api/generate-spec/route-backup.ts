...
Now create a high-converting landing page for: "${userPrompt}"`/server'
import { PageSpecSchema, CopySpecSchema } from '@/lib/schemas'
import { createClient } from '@supabase/supabase-js'
...
const SYSTEM_PROMPT = `You are a conversion-focused landing page planner. Output only VALID JSON that conforms to the provided JSON Schema. 
Do not produce HTML or Markdown. Do not invent new component types or fields. 
Choose from allowed components: Hero, FeaturesGrid, SocialProof, Pricing, Steps, FAQ, CTA, Footer.
Use no more than 6 sections. Ensure each chosen section includes all required fields.
When a section supports images, populate images with a query and orientation.
Honor the user's goals: 
- collect-waitlist -> must include Hero + CTA + FAQ
- pre-sell -> include Pricing
- book-calls -> include CTA with "Book a call"
Tone and palette should be consistent across the page.`

const PAGESPEC_PROMPT = (userPrompt: string) => `Task: Produce a PageSpec JSON for a single-page website.
...
Constraints:
- Use 4-6 sections only
- All section IDs must be strings
- All links must be strings
- Use valid hex colors starting with #
- Variant values: Hero("image-left"|"image-right"|"centered"), CTA("card"|"banner")`

const COPYSPEC_PROMPT = (pageSpecJson: string) => `Task: Produce CopySpec JSON for the given PageSpec.
...
Requirements:
- Output ONLY valid JSON, no other text
- Use the EXACT same section IDs from PageSpec
- Keep headlines punchy (<= 12 words)
- Example structure:

{
  "hero1": {
    "headline": "Your Amazing Product",
    "subhead": "Description of benefits",
  ...
  "cta1": {
    ...
    "subhead": "Join thousands of users",
    "cta": "Sign Up Now"
  },
  "faq1": [
    {
...
Maintain the brand tone from PageSpec.`
